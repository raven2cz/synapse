...
export function GenerationDataPanel({
    ...
    const otherMeta = [
        { label: t('genData.model'), value: meta.Model || meta.model_name, icon: Box },
        { label: t('genData.sampler'), value: meta.sampler, icon: Activity },
        { label: t('genData.steps'), value: meta.steps, icon: Layers },
        { label: t('genData.cfgScale'), value: meta.cfgScale || meta.cfg_scale, icon: Sliders },
        { label: t('genData.seed'), value: meta.seed ? String(meta.seed) : undefined, icon: Hash },
        { label: t('genData.clipSkip'), value: meta.clipSkip || meta.clip_skip, icon: Zap },
        { label: t('genData.size'), value: sizeValue, icon: Ruler },
        ...
        <div className={clsx("flex flex-col h-full bg-[#1a1b1e] text-text-primary overflow-hidden border-l border-white/5", className)}>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-8 custom-scrollbar bg-[#141517]">
                ...
                {prompt && (
                    <section className="space-y-3">
                        <div className="flex items-center justify-between">
                            <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest">{t('genData.prompt')}</h4>
                            <Button
                                variant="ghost"
                                size="sm"
                            ...
                            >
                                <Copy className="w-3 h-3 mr-1.5" /> {t('genData.copy')}
                            </Button>
                        </div>
                ...
                {negativePrompt && (
                    <section className="space-y-3">
                        <div className="flex items-center justify-between">
                            <h4 className="text-xs font-bold text-red-400/80 uppercase tracking-widest">{t('genData.negativePrompt')}</h4>
                            <Button
                                variant="ghost"
                                size="sm"
                ...
                {otherMeta.length > 0 && (
                    <section className="space-y-3">
                        <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest">{t('genData.params')}</h4>
