...
export const ImportWizardModal = memo<ImportWizardModalProps>(function ImportWizardModal({
  ...
  // Previews that will actually be downloaded (depends on option)
  const availablePreviews = useMemo(
    () => options.downloadFromAllVersions ? allVersionPreviews : selectedVersionPreviews,
    [options.downloadFromAllVersions, allVersionPreviews, selectedVersionPreviews]
  )
          ...
          {selectedVersionPreviews.length > 0 && (
            ...
            >
              ...
              <div className="grid grid-cols-4 gap-2">
                {selectedVersionPreviews.map((preview, idx) => {
                  const isNsfw = isPreviewNsfw(preview)
